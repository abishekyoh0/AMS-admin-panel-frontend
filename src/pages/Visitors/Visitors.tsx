...
const VisitorManagement: React.FC = () => {
  ...
  const filteredVisitors = visitors.filter((v) => {
    ...
    if (filter === "CHECKED IN")
      return v.status === "CHECKED IN" && matchSearch;
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex gap-3 mb-4 flex-wrap ${FONTSIZE[14]} ${FONTWEIGHT[400]}`}
      >
        <button
          ...
          <tbody>
            {filteredVisitors.map((v) => (
              <tr key={v.id} className="border-t border-white/10">
                ...
                <td
                  className={`p-3 ${FONTSIZE[14]} ${FONTWEIGHT[400]}`}
                  style={{ color: COLORS.secoundy_gray }}
                >
                  {v.phone}
                </td>
                ...
                <td className="p-3 flex gap-2">
                  <button
                    ...
                    className={`bg-[#2B7FFF33] text-[#51A2FF] px-3 py-1 rounded-2xl ${FONTSIZE[12]} ${FONTWEIGHT[400]} cursor-pointer`}
                  >
                    View
                  </button>
                  {v.status === "CHECKED IN" && (
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="relative bg-[#101828F2] w-125 rounded-2xl border border-[#FFFFFF33] p-6">
            ...
            <div
              className={`grid grid-cols-2 gap-4 mb-4 ${FONTSIZE[18]} ${FONTWEIGHT[400]}`}
            >
              <div>
                <p
                  className={`mb-1 ${FONTSIZE[14]} ${FONTWEIGHT[400]}`}
                  style={{ color: COLORS.secoundy_gray }}
                >
                  Purpose
                </p>
