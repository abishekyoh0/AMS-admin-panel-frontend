...
const STATS: StatCard[] = [
  {
    ...
    value: 5,
    color: "#60a5fa",
    bg: "bg-linear-to-r from-[#2B7FFF1A] to-[#00B8DB1A] border-[#51A2FF4D]",
    icon: Alarm,
    iconBg: "#2B7FFF33",
  },
  {
    ...
    value: 3,
    color: "#22c55e",
    bg: "bg-linear-to-r from-[#00C9501A] to-[#00BC7D1A] border-[#05DF724D]",
    icon: Tick,
    iconBg: "#00C95033",
  },
  {
    ...
    value: 1,
    color: "#f59e0b",
    bg: "bg-linear-to-r from-[#F0B1001A] to-[#FE9A001A] border-[#FDC7004D]",
    icon: Clock,
    iconBg: "#F0B10033",
  },
  {
    ...
    value: 1,
    color: "#ef4444",
    bg: "bg-linear-to-r from-[#FB2C361A] to-[#FF20561A] border-[#FF64674D]",
    icon: Close,
    iconBg: "#FB2C3633",
  },
  {
    ...
    value: 1200,
    color: "#a855f7",
    bg: "bg-linear-to-r from-[#AD46FF1A] to-[#F6339A1A] border-[#C27AFF4D]",
    icon: Send,
    iconBg: "#AD46FF33",
  },
  {
    ...
    value: 842,
    color: "#06b6d4",
    bg: "bg-linear-to-r from-[#00B8DB1A] to-[#00BBA71A] border-[#00D3F34D]",
    icon: Eye,
    iconBg: "#00B8DB33",
  },
];
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-center p-4">
      <div className="w-full max-w-3xl max-h-125 overflow-y-auto bg-linear-to-br from-[#0F172B] to-[#101828] border border-[#FFFFFF33] rounded-2xl p-6 text-white relative">
        ...
        <div className="flex flex-wrap gap-2 mb-5 text-xs">
          ...
          <span
            className={`flex items-center px-2 py-1 rounded-xl ${item.status == "Scheduled" ? "bg-[#2B7FFF33] text-[#51A2FF]" : "bg-[#00C95033] text-[#05DF72]"}`}
          >
            {item.status == "Scheduled" ? (
              <img src={Schedule} alt="" className="w-4 h-4 mr-1 inline" />
            ) : (
              <img src={Published} alt="" className="w-4 h-4 mr-1 inline" />
            )}
            {item.status}
          </span>
          <span
            className="flex gap-2 items-center bg-linear-to-r from-[#F0B100] to-[#E17100] px-4 py-1 rounded-lg text-sm cursor-pointer"
            style={{
              boxShadow:
  ...
  return (
    <div style={{ color: COLORS.primary_white }}>
        ...
        <div className="flex items-center gap-3">
          <div>
            <img
              src={Notify}
              alt=""
              className="bg-linear-to-r from-[#AD46FF33] to-[#F6339A33] p-2 border border-[#C27AFF4D] rounded-xl"
            />
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        {STATS.map((card) => (
          <div key={card.id} className={`rounded-xl p-4 border-2 ${card.bg}`}>
            <h2
              className={`flex justify-between items-center mt-1 mb-5 ${FONTSIZE[24]} ${FONTWEIGHT[700]}`}
        ...
        {filtered.map((item) => {
          ...
          return (
            <div
              ...
              style={{
                boxShadow:
                  "0px 4px 6px -4px #F0B1001A,0px 10px 15px -3px #F0B1001A",
              }}
            >
              <img
                src={Notify}
                alt=""
                className="bg-linear-to-r from-[#AD46FF33] to-[#F6339A33] p-2 border border-[#C27AFF4D] rounded-xl w-15 h-15"
              />
              <div className="w-full">
                <div
                  className={`flex items-center flex-wrap gap-2 mt-2 ${FONTSIZE[12]} ${FONTWEIGHT[700]}`}
                >
                  <img src={item.icon} alt="" className="w-5 h-5" />
                  <h2 className={`${FONTSIZE[20]} ${FONTWEIGHT[700]}`}>
                    {item.title}
                  </h2>
                  <span
                    className={`bg-red-500/20 text-red-400 px-2 py-1 rounded-xl `}
                  >
                    <img src="" alt="" /> {item.tag}
                  </span>
                  <span
                    className={`px-2 py-1 rounded-xl ${badgeColor(item.priority)}`}
                  >
                    {item.priority}
                  </span>
                  <span
