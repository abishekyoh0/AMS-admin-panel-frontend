...
export default function RejectModal({
  ...
  const handleSubmit = () => {
    if (!reason.trim()) {
      setError("Please provide a clear reason for rejection to help the resident understand.");
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-3 sm:px-4">
      ...
      <div className="w-full max-w-md sm:max-w-lg rounded-2xl p-4 sm:p-6 bg-linear-to-br from-[#0F172A] to-[#111827] border border-white/10 shadow-2xl">
        ...
        {error && (
          <div className={`mt-3 text-xs sm:text-sm text-[#FF6467] bg-[#FB2C361A] border border-[#FF64674D] p-2 rounded-lg ${FONTSIZE[14]}`} style={{ fontWeight: WEIGHT.four }}>
            {error}
          </div>
        )}
